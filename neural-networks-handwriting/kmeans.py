...
class KMeans:
    ...
    def dist_pt_to_centroids(self, pt, centroids):
        ...
        only a small fraction of the time for your code to stop running)
        '''
        distances = np.linalg.norm(pt[:, np.newaxis, :] - centroids, axis=2)
        return distances
    ...
    def initialize(self, k):
        ...
        '''
        self.k = k
        return self.data[np.random.choice(self.num_samps, k, replace=False)]
    ...
    def cluster(self, k=2, tol=1e-2, max_iter=1000, verbose=False, p=2):
        ...
        for _ in range(max_iter):
            ...
            centroids, centroid_diff = self.update_centroids(k, self.data_centroid_labels, prev_centroids)
            if np.max(np.abs(centroid_diff)) < tol: break
        self.centroids = centroids
        self.inertia = self.compute_inertia()
        if verbose: print("Iterations:", _ + 1)
        return self.inertia
    ...
    def cluster_batch(self, k=2, n_iter=1, verbose=False, p=2):
        ...
        self.centroids, self.data_centroid_labels, self.inertia = best_centroids, best_labels, best_inertia
    ...
    def update_labels(self, centroids):
        ...
        '''
        return np.argmin(self.dist_pt_to_centroids(self.data, centroids), axis=1)
    ...
    def update_centroids(self, k, data_centroid_labels, prev_centroids):
        ...
        '''
        new_centroids = np.array([self.data[data_centroid_labels == i].mean(axis=0) if np.any(data_centroid_labels == i) else self.data[np.random.choice(self.num_samps)] for i in range(k)])
    ...
    def compute_inertia(self):
        ...
        '''
        return np.mean(np.min(self.dist_pt_to_centroids(self.data, self.centroids), axis=1) ** 2)
    ...
    def plot_clusters(self):
        ...
        '''
        plt.scatter(self.data[:, 0], self.data[:, 1], c=self.data_centroid_labels, cmap='tab10')
        plt.scatter(*self.centroids.T, c='black', marker='X')
        plt.show()
    ...
    def elbow_plot(self, max_k):
        ...
        '''
        inertias = [self.cluster(k) for k in range(1, max_k + 1)]
