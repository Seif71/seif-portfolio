...
class RBF_Net:
    ...
    def avg_cluster_dist(self, data, centroids, cluster_assignments, kmeans_obj):
        ...
        average_distance = np.zeros(num_clusters)
        
        for i in range(num_clusters):
            assigned_data = data[cluster_assignments == i]
            if len(assigned_data) > 0:  
                average_distance[i] = np.mean(np.linalg.norm(assigned_data - centroids[i], axis=1))
    ...
    def initialize(self, data):
        ...
        '''
        kmeans_obj = KMeans(n_clusters=self.k, max_iter=5) 
        kmeans_obj.fit(data)
        self.prototypes = kmeans_obj.cluster_centers_
        cluster_assignments = kmeans_obj.predict(data) 
    ...
    def linear_regression(self, A, y):
        ...
        return np.linalg.lstsq(A_with_bias, y, rcond=None)[0]  
    ...
    def hidden_act(self, data):
        ...
        '''
        distances = np.linalg.norm(data[:, None] - self.prototypes, axis=2)  
    ...
    def output_act(self, hidden_acts):
        ...
        '''
        return hidden_acts @ self.wts[:-1, :] + self.wts[-1, :]
    ...
    def train(self, data, y):
        ...
        self.wts = self.linear_regression(hidden_acts, np.eye(self.num_classes)[y]) 
